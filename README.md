...
___상관 관계___
  * X값이 증가하면 Y값이 비례해서 증가하거나, X값이 증가하면 Y값이 비례해서 감소하는 경우 두 변수는 [상관관계]에 있다고 한다.
  * 두 변수가 어느정도 강한 상관관계인지 나타낸 지표를 [상관계수]라고 한다.
  * 상관계수가 0인 두 변수는 사이의 상관성을 찾기 어렵다.
  * 상관계수가 -인 경우는 [음의 상관관계], +인 경우는 [양의 상관관계]에 있다고 말한다.
```R
#1. 데이터 확인
head(cars)
...
### ___데이터 전처리___
  * 현실에선 확보한 데이터를 정제, 가공해야 분석에 적합한 데이터가 되는데, 이 과정을 [데이터 전처리]라고 한다.
  * 데이터 전처리는 전체 분석 과정에서 오랜 시간을 차지한다.
  * [결측값]은 데이터의 수집, 저장 과정에서 값을 얻지 못하는 경우 발생한다.
  * 결측값은 R에서 NA로 표현되며, 데이터를 분석할 때 여러 문제를 일으킨다.
  * 결측값은 제거, 제외하고 분석하거나 적당한 값으로 치환한 후 분석한다.

___벡터의 결측값___
```R
z <- c(1, 2, 3, NA, 5, NA, 8) #결측값이 포함된 벡터 z
sum(z)                        #[1] NA, 정상적으로 실행되지 않음
is.na(z)                      #NA 여부 확인
sum(is.na(z))                 #NA 개수 확인
sum(z, na.rm=TRUE)            #NA를 제외하고 합계 계산
```
```R
#NA를 다른 값으로 대체하거나 제거하는 방법
...
#행 별로 결측값의 개수 구하기
rowSums(is.na(x))         #행 별 NA 개수
sum(rowSums(is.na(x))>0) #NA가 포함된 행의 개수
sum(is.na(x))            #데이터셋 전체에서 NA 개수

#NA를 제외한 완전한 행 찾기
head(x)                    #NA가 포함된 행들을 나타냄
x[!complete.cases(x),]     #NA가 포함된 행들을 제거
y <- x[complete.cases(x),] #새로운 데이터셋 y의 내용 확인
head(y)
```
